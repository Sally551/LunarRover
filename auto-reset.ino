...
const int AUTO_RESET_INTERVAL = 4000;
const int IR_LATCH_THRESHOLD = 3;
...
// ================= ROCK CLASSIFIER =================
void classifyRock() {
  if (rockType == "none") return;  // no rock present
...
void updateIR() {
  ...
  if (now - irLastSample >= IR_SAMPLE_WINDOW) {
    ...
    irPulseCount = 0;
    irLastSample = now;
    int rate     = count * (1000 / IR_SAMPLE_WINDOW);

    Serial.println("IR sample: " + String(rate) + " /s (latched: " + String(irLatched) + ")");

     if (rate < 200) {
       Serial.println("No rock detected — resetting");
       doReset();
       rockType = "none";
       irRate   = String(rate);
       return;
    ...
    if (!irLatched) {
      if (rate > 493) {
        irHighCount++;
        irLowCount = 0;
        Serial.println("High count: " + String(irHighCount) + "/" + String(IR_LATCH_THRESHOLD));
      } else if (rate < 271) {
        irLowCount++;
        irHighCount = 0;
        Serial.println("Low count: " + String(irLowCount) + "/" + String(IR_LATCH_THRESHOLD));
      } else {
        Serial.println("IR ambiguous: " + String(rate));
      }

      if (irHighCount >= IR_LATCH_THRESHOLD) {
        irRate    = "547";
        irLatched = true;
        Serial.println("IR LATCHED: HIGH (547)");
        classifyRock();
      } else if (irLowCount >= IR_LATCH_THRESHOLD) {
        irRate    = "312";
        irLatched = true;
        Serial.println("IR LATCHED: LOW (312)");
        classifyRock();
      }
    }
  }
}
// Core reset logic — safe to call from anywhere
void doReset() {
  irHighCount        = 0;
  irLowCount         = 0;
  irLatched          = false;
  irRate             = "---";
  ultrasonicDetected = "unknown";
  magneticDir        = "unknown";
  rockAge            = "----";
  rockType           = "unknown";
  Serial.println("Sensors reset — ready for new rock");
}

// Server handler — called when browser hits /reset
void resetSensors() {
  doReset();
...
void runTestMode() {
  ...
  else if (cmd == 'x') { resetSensors(); return; }
  else if (cmd == 'a') {
...
void setup() {
 ...
 Serial.println(ip[3]);
  ...
  server.on(F("/reset"), resetSensors);
