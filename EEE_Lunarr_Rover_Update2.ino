...
// Minimal WiFi reconnect timer
unsigned long lastWiFiReconnect = 0;
unsigned long lastClientContact = 0;
const unsigned long CLIENT_TIMEOUT = 1000;
...
// ================= LOOP =================
void loop() {
  if (motorsRunning && millis() - lastClientContact > CLIENT_TIMEOUT) {
  ...
  if (WiFi.status() != WL_CONNECTED &&
    ...
    // Stop rover if control connection is lost
    analogWrite(leftEN, 0);
    analogWrite(rightEN, 0);
    motorsRunning = false;
